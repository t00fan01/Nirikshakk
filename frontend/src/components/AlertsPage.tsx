            ...
            <section className="bg-white rounded-3xl border border-slate-200 shadow-[0_6px_32px_rgba(147,111,173,0.12)] overflow-hidden">
                <div className="p-5 border-b border-slate-200 bg-[#F8FAFC]/80 flex flex-col xl:flex-row gap-3 xl:items-center xl:justify-between">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3">
                        <div className="relative"><Search className="w-4 h-4 text-slate-400 absolute left-3 top-1/2 -translate-y-1/2" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search alerts or accounts" className="w-full sm:w-64 bg-white border border-slate-200 focus:border-[#FF4F00] rounded-xl pl-9 pr-3 py-2 text-xs font-medium text-[#002A24] outline-none" /></div>
                        ...
                        <select value={classification} onChange={(event) => setClassification(event.target.value)} className="bg-white border border-slate-200 focus:border-[#FF4F00] rounded-xl px-3 py-2 text-xs font-bold text-[#002A24] outline-none"><option value="ALL">All classifications</option><option value="MULE_SUSPECTED">Mule suspected</option><option value="SUSPICIOUS">Suspicious</option></select>
                    </div>
                </div>

                {loading ? <div className="flex items-center justify-center py-16"><Loader2 className="w-6 h-6 text-[#FF4F00] animate-spin mr-3" /><span className="text-xs font-black uppercase tracking-widest text-slate-500">Loading alerts...</span></div> : error ? <div className="p-8 text-center bg-rose-50 text-rose-700 text-sm font-medium">{error}</div> : filteredAlerts.length === 0 ? <div className="p-12 text-center"><ShieldAlert className="w-8 h-8 text-slate-300 mx-auto mb-3" /><p className="text-sm font-bold text-slate-500">No alerts match the current filters.</p></div> : <div className="overflow-x-auto"><table className="min-w-full divide-y divide-slate-200"><thead className="bg-[#F8FAFC]"><tr>{['Alert ID', 'Time', 'Account', 'Severity', 'Classification', 'Risk score', 'Alert type', 'Reason', 'Status'].map((heading) => <th key={heading} className="px-5 py-3 text-left text-[10px] font-black uppercase tracking-widest text-slate-500">{heading}</th>)}</tr></thead><tbody className="divide-y divide-slate-200">{filteredAlerts.map((alert) => <tr key={alert.alert_id} onClick={() => alert.account_id && navigate(`/dashboard?account=${encodeURIComponent(alert.account_id)}`)} className={`transition-colors ${alert.account_id ? 'cursor-pointer hover:bg-slate-50' : ''}`}><td className="px-5 py-4 text-xs font-black text-[#002A24]">{alert.alert_id ?? 'Unknown'}</td><td className="px-5 py-4 text-xs font-medium text-slate-600 whitespace-nowrap">{formatDate(alert.timestamp)}</td><td className="px-5 py-4 text-xs font-black text-[#002A24]"><button type="button" onClick={(event) => { event.stopPropagation(); if (alert.account_id) navigate(`/dashboard?account=${encodeURIComponent(alert.account_id)}`); }} className="font-black text-[#002A24] hover:text-[#FF4F00] transition-colors">{alert.account_id ?? 'Unknown'}</button></td><td className="px-5 py-4"><span className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-widest ${severityStyle(alert.severity)}`}>{alert.severity ?? 'Unknown'}</span></td><td className="px-5 py-4 text-xs font-bold text-slate-600">{alert.classification ?? 'Unknown'}</td><td className="px-5 py-4 text-xs font-black text-[#002A24]">{typeof alert.risk_score === 'number' ? alert.risk_score.toFixed(1) : 'Unknown'}</td><td className="px-5 py-4 text-xs font-bold text-slate-600">{alert.alert_type ?? 'Unknown'}</td><td className="px-5 py-4 text-xs font-medium text-slate-600">{alert.reason ?? 'Unknown'}</td><td className="px-5 py-4 text-xs font-black uppercase tracking-widest text-slate-500">{alert.status ?? 'Unknown'}</td></tr>)}</tbody></table></div>}
            </section>

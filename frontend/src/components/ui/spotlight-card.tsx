...
const GlowCard: React.FC<GlowCardProps> = ({ 
  ...
  useEffect(() => {
    const syncPointer = (e: PointerEvent) => {
      ...
      if (cardRef.current) {
        // We need coordinates relative to the viewport for fixed background attachment 
        // but the component style uses window coordinates.
  ...
  const beforeAfterStyles = `
    ...
    .glow-card[data-glow] .inner-glow {
      position: absolute;
      inset: 0;
      will-change: filter;
      opacity: var(--outer, 1);
  ...
  return (
    <>
      ...
      <div
        ...
        `}
      >
        <div className="inner-glow" data-glow></div>
